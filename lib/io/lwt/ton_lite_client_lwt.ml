...
let fail_all t reason =
  ...
  Hashtbl.iter
    (fun _ u -> try Lwt.wakeup_later_exn u (Failure reason) with _ -> ())
    t.pending;
  Hashtbl.reset t.pending;
  List.iter
    (fun u -> try Lwt.wakeup_later_exn u (Failure reason) with _ -> ())
    t.pongs;
  t.pongs <- []
...
let rec read_loop t =
  Lwt.catch
    (fun () ->
      ...
      if n = 0 then (
        fail_all t "peer closed the connection";
        Lwt.return_unit)
      else
  ...
  Lwt.catch
    (fun () ->
      ...
      | Ok (conn, packet) ->
          ...
          let rec wait n =
            if Ton_adnl.Conn.confirmed (C.Session.conn t.session) then
              Lwt.return_true
      ...
      Lwt.catch
        (fun () ->
          ...
          Lwt.pick
            [
              (waiter >|= fun v -> Ok v);
              (Lwt_unix.sleep timeout >|= fun () -> Error Timeout);
            ])
        (fun exn -> Lwt.return (Error (Closed (Printexc.to_string exn))))
...
let call ?(timeout = 30.0) t q =
  ...
  t.session <- session;
  await ~timeout t ~bytes ~register:(fun u ->
      Hashtbl.replace t.pending query_id u)
  >|= function
  | Error e -> Error e
  | Ok body -> (
