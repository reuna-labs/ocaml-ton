...
let field n = match List.assoc_opt n (Lazy.force transcript) with
  | Some v -> v
  | None -> Alcotest.failf "missing field %S" n

let str n = match field n with `String s -> unhex s | _ -> Alcotest.failf "%s" n
let strs n =
  match field n with
...
module Replay = struct
  ...
  type flow = { mutable reads : string list; mutable writes : string list; mutable closed : bool }

  let make reads = { reads; writes = []; closed = false }
  ...
  let close f = f.closed <- true; Lwt.return_unit
end
...
let scripted () =
  let queue = ref [ str "ephemeralSeed"; str "aesParams"; str "queryId"; str "nonce" ] in
  fun n ->
    match !queue with
...
let test_handshake () =
  ...
  match run (T.connect ~random:(scripted ()) flow ~server_pub:(str "serverPub")) with
  | Error e -> Alcotest.failf "%a" Ton_lite_client_mirage.pp_error e
  | Ok _ ->
      Alcotest.(check string) "the handshake packet"
...
let test_query () =
  ...
  let result =
    run
      ( T.connect ~random:(scripted ()) flow ~server_pub:(str "serverPub") >>= function
  ...
  | Error e -> Alcotest.failf "%a" Ton_lite_client_mirage.pp_error e
  | Ok info ->
      Alcotest.(check string) "the query frame"
        (hex (List.nth (strs "clientWrites") 1))
        (hex (List.nth flow.Replay.writes 1));
      let last = info.Ton_lite_client.Lite.last in
      Alcotest.(check int32) "masterchain workchain" (-1l) last.Ton_lite_client.Lite.workchain;
      Alcotest.(check bool) "a plausible seqno" true (last.Ton_lite_client.Lite.seqno > 80_000_000l)
...
let test_eof () =
  ...
  | Error e ->
      Alcotest.(check string) "reported" "connection closed: peer closed the connection"
...
let test_dribble () =
  ...
  | Error e -> Alcotest.failf "%a" Ton_lite_client_mirage.pp_error e
  | Ok info ->
      Alcotest.(check bool) "still decoded" true
